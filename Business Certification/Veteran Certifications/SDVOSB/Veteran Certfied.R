...
dsbs <- read_excel("DSBS SDVOSB.xlsx")
neoserra <- read_excel("Neoserra Clients.xlsx")

dsbs$`Service-Disabled Veteran-Owned Cert Date` <- ymd(dsbs$`Service-Disabled Veteran-Owned Cert Date`)
dsbs$`Service-Disabled Veteran-Owned Expiration Date` <- ymd(dsbs$`Service-Disabled Veteran-Owned Expiration Date`)
...
matches <- data.frame(Client_ID = character(),
                      ...
                      `Company Veteran Status` = character(),
                      `Veteran-Owned Cert Number` = character(),
                      `Veteran-Owned Cert Date` = Date(),
                      `Veteran-Owned Expiration Date` = Date(),
                      `Primary Consultants` = character(),
                      dsbs_name = character(),
...
for (i in 1:nrow(non_id_clients)) {
  ...
  for (j in 1:nrow(dsbs)) {
    ...
    if (distance < min_distance) {
      ...
      matched_name <- name2
      vet_number <- dsbs$`Veteran-Owned Cert Number`[j]
      vet_date <- dsbs$`Service-Disabled Veteran-Owned Cert Date`[j]
      vet_exp <- dsbs$`Service-Disabled Veteran-Owned Expiration Date`[j]
    }
  }
  ...
  if (min_distance <= threshold) {
    matches <- rbind(matches, data.frame(Client_ID = non_id_clients$`Client ID`[i],
                                         ...
                                         `Company Veteran Status` = non_id_clients$`Company Veteran Status`[i],
                                         `Service-Disabled Veteran-Owned Cert Number` = vet_number,
                                         `Service-Disabled Veteran-Owned Cert Date` = vet_date,
                                         `Service-Disabled Veteran-Owned Expiration Date` = vet_exp,
                                         `Primary Consultants` = non_id_clients$`Primary Consultants`[i],
...
disadvantaged <- joined %>%
  ...
  select(`Client ID`, `Company Name`, `Unique Entity Identifier`, `Company Veteran Status`,
         `Service-Disabled Veteran-Owned Cert Number`, `Service-Disabled Veteran-Owned Cert Date`, 
         `Service-Disabled Veteran-Owned Expiration Date`, `Primary Consultants`)

# Combine the two
combined_certify <- bind_rows(disadvantaged, filtered_matches)

# Export the final data set
write_xlsx(combined_certify, "SDVOSB Certified.xlsx")


# Find certified clients that shouldn't be certified 
# Clients that have a UEI
dont_certify <- uei_clients %>%
  filter(`Company Veteran Status` == "SDVOSB Certified")
...
non_id_clients_certified <- non_id_clients %>%
  filter(`Company Veteran Status` == c("SDVOSB Certified"))
...
write_xlsx(non_matches, "Not SDVOSB.xlsx")
