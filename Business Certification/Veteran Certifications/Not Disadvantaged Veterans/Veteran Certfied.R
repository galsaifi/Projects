...
dsbs <- read_excel("DSBS Veteran Owned.xlsx")
...
matches <- data.frame(Client_ID = character(),
                      ...
                      `Veteran Certification Date` = character(),
                      `Veteran-Owned Cert Number` = character(),
                      `Veteran-Owned Cert Date` = character(),
                      `Veteran-Owned Expiration Date` = character(),
...
for (i in 1:nrow(non_id_clients)) {
  ...
  for (j in 1:nrow(dsbs)) {
    ...
    if (distance < min_distance) {
      ...
      vet_date <- dsbs$`Veteran-Owned Cert Date`[j]
      vet_exp <- dsbs$`Veteran-Owned Expiration Date`[j]
    }
  }
  ...
  if (min_distance <= threshold) {
    matches <- rbind(matches, data.frame(Client_ID = non_id_clients$`Client ID`[i],
                                         ...
                                         `Veteran Certification Date` = non_id_clients$`Veteran Certification Date`[i],
                                         `Veteran-Owned Cert Number` = vet_number,
                                         `Veteran-Owned Cert Date` = vet_date,
                                         `Veteran-Owned Cert Expiration Date` = vet_exp,
...
disadvantaged <- joined %>%
  ...
  select(`Client ID`, `Company Name`, `Unique Entity Identifier`, `Company Veteran Status`, `Veteran Certification Date`,
         `Veteran-Owned Cert Number`, `Veteran-Owned Cert Date`, `Veteran-Owned Expiration Date`,
         `Primary Consultants`)
...
write_xlsx(disadvantaged, "Veteran Owned Certified.xlsx")


# Find certified clients that shouldn't be certified 
# Clients that have a UEI
dont_certify <- uei_clients %>%
  filter(`Company Veteran Status` == "VOSB Certified")
...
non_id_clients_certified <- non_id_clients %>%
  filter(`Company Veteran Status` == c("VOSB Certified"))
...
write_xlsx(non_matches, "Not Veteran Owned.xlsx")
